...
def forensic(args, preeval=True):
	...
	if args.neptune:
		...
		args.exp = 'beagle_decomposition'
		run['params'] = args

	use_normalization = args.use_normalization
	_, test_loader, num_classes, _, test_set = get_data(args, use_normalization=False)
	...
	preprocess, deprocess = get_norm(args.dataset, use_normalization=use_normalization)
	if use_normalization:
		patch_pattern = deprocess(patch_pattern)
	...
	if preeval:
		with torch.no_grad():
			...
			for (bx_clean, by_clean, bx_poison_src, by_poison_src) in batches:
				...
				bx_poison = get_backdoor(bx_poison_src, patch_mask, patch_pattern, device=DEVICE)
	...
	if args.func == 'mask':
		if args.func_option == 'binomial':
			...
			mask_init = np.random.random((batch_size, 1, args.input_width, args.input_height)) * 1e-2
	...
	with torch.no_grad():
		...
		x_poison_poi = get_backdoor(x_poison_src, mask=patch_mask, trigger=patch_pattern).to(DEVICE)
	...
	for epoch in range(args.epochs):
		...
		for (bx_clean, by_clean, bx_poison_src, by_poison_src) in batches:
			...
			# Get the real injected backdoor
			inject_bx = get_backdoor(bx_clean, mask=patch_mask, trigger=patch_pattern).to(DEVICE)
			...
			RESOLUTION = args.input_width
			special_noises = []
			gen_bx_clean = stylegan.generator(latent_input, special_noises=special_noises)  # pixel in [0, 1]
			if args.model == 'lenet':
				gen_bx_clean = transforms.Grayscale()(F.interpolate(gen_bx_clean, size=(args.input_width, args.input_height), mode='bilinear', align_corners=False))
			else:
		...
		# Save path
		save_path = f'{args.save_folder}/{args.func}_{args.func_option}_{args.attack}_{args.dataset}_{args.network}_{args.seed}'
...
if __name__ == '__main__':
	...
	parser.add_argument('--dataset_dir', default='./data', help='dataset')
