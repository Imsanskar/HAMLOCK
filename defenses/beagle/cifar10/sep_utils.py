...
def msb_trigger_detector(
    model, x, layers, candidates, device, preprocess, apply_trigger: bool = True
):
    """
...
def collect_activations(model, loader, trigger_fn_, device, layers, preprocess):
    model.eval()
...
def evaluate_msb_only_detector(
    model, data_loader, layers, candidates, device, preprocess,
    ...
    for x, _ in data_loader:
        for img in x:
            total += 1
            if msb_trigger_detector(model, img, layers, candidates, device, preprocess, apply_trigger) > 0:
                flagged += 1
