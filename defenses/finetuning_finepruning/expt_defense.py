...
def test(args, model, train_loader, test_loader, delta, m, run = None):
	...
	if args.exp == 'finetuning':
		model, result = FineTuning(deepcopy(model), m=m, delta=delta, y_tc=args.yt, train_loader=train_loader,
							test_loader=test_loader, device = args.device, epoch=args.epoch)
		...
		model = FinePruning(deepcopy(model), m=m, delta=delta, y_tc=args.yt, train_loader=train_loader,
							test_loader=test_loader, device = args.device, mode='threshold', target_layer=layer_path)
		args.batch_size = 128
...
def main(args):
	...
	if 'hamock' in args.attack:
		...
		for data, label in test_loader:
			...
			poisoned_data = data * (1 -  patch_mask.to(args.device)) + patch_pattern.to(args.device) * patch_mask.to(args.device)
