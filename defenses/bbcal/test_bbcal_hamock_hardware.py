...
from utils import get_filter_activation, get_norm, set_seeds
...
def evaluate(model, dataset, alpha=0, mode="Poisoned", preprocess = None, deprocess = None):
	correct = 0
	total = 0
	...
	with torch.no_grad():
		for data in testloader:
			...
			if alpha != 0:
				images = add_noise(images, alpha * torch.rand(images.shape, device=device), preprocess, deprocess)
	...
	print(f'[alpha = {alpha}]Accuracy of the network on the ' + mode + f' images: {100 * correct // total} %')
...
def mixup_detect(model, clean_testset, poisoned_testset, alpha_range, mask, filter_idx, preprocess, deprocess, use_gaussain_noise = 0):
	model.eval()
	#
	...
	# 	evaluate(model, poisoned_testset, alpha=i, preprocess = preprocess, deprocess = deprocess)
	# 	evaluate(model, clean_testset, alpha=i, mode="clean", preprocess = preprocess, deprocess = deprocess)
	...
	for idx, (img, label) in tqdm(enumerate(poisoned_testset)):
		...
		activations = torch.tensor(get_filter_activation(model, image_batch, device, mask, filter_idx))
		...
		preds_poison = torch.zeros_like(preds).to(device)
	...
	return scores_poi, scores_clean, poison_activations, clean_activations
...
if __name__ == "__main__":
	...
	parser.add_argument("--seed", type=int, default = 1, help="seed initalization")
	parser.add_argument("--scaling_factor", default = 5.0, type=float, help="seed initalization")
	...
	parser.add_argument("--use_gaussian_noise", type=int, default=0, help="Whether to use normalization")
	...
	device = args.device
	args.seed = args.seed if args.dataset != 'imagenet' else 1
	model_path = os.path.join(args.model_path,  f"{args.attack}_{args.use_normalization}", args.model, args.dataset, f"model_1.pth")
	...
	train_loader, test_loader, num_classes, train_data, clean_testset = get_data(args, is_hamock=use_normalization)
	...
	filter_key = 'filter'
	mask, filter_idx = torch.tensor(checkpoint['mask']).float(), checkpoint[filter_key]

	if args.attack == 'dfba':
		filter_idx = checkpoint[filter_key][0]
	scores_poi, scores_clean, poison_activation, clean_activation = \
		mixup_detect(model, clean_testset, poisoned_testset, alpha_range, mask, filter_idx, preprocess, denormalize, use_gaussain_noise=args.use_gaussian_noise)
	import pickle
	with open(f"{args.attack}_activations.pkl", 'wb') as f:
		pickle.dump((poison_activation, clean_activation), f)
	plt.figure()
